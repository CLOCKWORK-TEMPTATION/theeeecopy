...
interface ManifestData {
  buildManifest?: any;
  appBuildManifest?: any;
  routesManifest?: any;
  middlewareManifest?: any;
}
...
function generateRoleSummary(filePath: string, category: string, exports: string[]): string {
  ...
  if (category === 'app/page') {
    const routePath = filePath.match(/src[\\/]app[\\/](.*)[\\/]page\./)?.[1]?.replace(/[\\/]/g, '/') || '';
    return `Next.js App Router page for route: /${routePath}`;
  }
  if (category === 'app/layout') {
    const routePath = filePath.match(/src[\\/]app[\\/](.*)[\\/]layout\./)?.[1]?.replace(/[\\/]/g, '/') || '';
    return `Layout component providing shared UI for: /${routePath}`;
  }
  if (category === 'app/route') {
    const routePath = filePath.match(/src[\\/]app[\\/](.*)[\\/]route\./)?.[1]?.replace(/[\\/]/g, '/') || '';
...
function determineStatus(filePath: string, category: string): string {
...
function calculateConfidence(info: FileInfo, manifests: ManifestData, allFiles: Map<string, FileInfo>): number {
  let confidence = 0;
...
async function loadManifests(): Promise<ManifestData> {
  ...
  for (const file of manifestFiles) {
    ...
    if (await fileExists(filePath)) {
      try {
        ...
        manifests[key as keyof ManifestData] = JSON.parse(content);
      } catch (error) {
...
function findImporters(targetPath: string, allFiles: Map<string, FileInfo>): string[] {
  const importers: string[] = [];
  const normalizedTarget = targetPath.replace(/\\/g, '/').replace(/\.(tsx?|jsx?)$/, '');

  for (const [filePath, info] of allFiles.entries()) {
    ...
    // Check if this file might import the target
    if (filePath.match(/\.(tsx?|jsx?)$/)) {
      importers.push(filePath);
    }
  }

  return importers.slice(0, 10); // Limit to 10 for brevity
}

function mapToRoutes(filePath: string, manifests: ManifestData): string[] {
  const routes: string[] = [];
  ...
  // Check manifests
  if (manifests.buildManifest?.pages) {
    for (const [page, files] of Object.entries(manifests.buildManifest.pages)) {
      if (Array.isArray(files) && files.some((f: string) => f.includes(path.basename(filePath)))) {
        routes.push(page);
      }
    }
  }

  if (manifests.appBuildManifest?.pages) {
...
async function analyzeAllFiles(): Promise<Map<string, FileInfo>> {
...
function generateMarkdownReport(files: FileInfo[], stats: AnalysisStats): string {
  const md: string[] = [];
...
function generateSummary(stats: AnalysisStats): string {
  const md: string[] = [];
...
async function generateReport() {
  ...
  for (const [filePath, info] of fileMap.entries()) {
    ...
    info.confidence = calculateConfidence(info, manifests, fileMap);

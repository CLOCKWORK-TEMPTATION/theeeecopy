...
describe('ShotsPage', () => {
  it('renders the initial state with no scene selected', () => {
    ...
    expect(screen.getByRole('heading', { name: /اللقطات/i })).toBeInTheDocument();
  ...
  it('renders a list of shots when a scene is selected', async () => {
    const mockShots = [
      { id: '1', shotNumber: 1, shotType: 'Wide', cameraAngle: 'Eye Level', cameraMovement: 'Static', lighting: 'Daylight', aiSuggestion: '' },
      { id: '2', shotNumber: 2, shotType: 'Close-up', cameraAngle: 'High Angle', cameraMovement: 'Pan', lighting: 'Night', aiSuggestion: 'A suggestion' },
    ];
    vi.mocked(useQuery).mockImplementation(({ queryKey }) => {
      if (queryKey[0] === 'scenes') {
        return { data: [{ id: '1', sceneNumber: 1, title: 'Scene 1' }], isLoading: false };
      }
      if (queryKey[0] === 'shots') {
  ...
  it('renders a message when there are no shots for the selected scene', async () => {
    ...
    expect(await screen.findByText(/لا توجد لقطات لهذا المشهد حتى الآن/)).toBeInTheDocument();
  });
});

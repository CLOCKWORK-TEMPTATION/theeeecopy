...
describe('ScenesPage', () => {
  it('renders the initial state with no scenes', () => {
    ...
    expect(screen.getByRole('heading', { name: /المشاهد/i })).toBeInTheDocument();
    expect(screen.getByText(/لا توجد مشاهد حتى الآن/)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /إنشاء مشهد جديد/i })).toBeInTheDocument();
  });

  it('renders a list of scenes', () => {
    const mockScenes = [
      { id: '1', sceneNumber: 1, title: 'Scene 1', location: 'Location 1', timeOfDay: 'Day', characters: ['Character 1'], shotCount: 2, status: 'planned', description: '' },
      { id: '2', sceneNumber: 2, title: 'Scene 2', location: 'Location 2', timeOfDay: 'Night', characters: ['Character 2'], shotCount: 1, status: 'completed', description: 'A description' },
    ];
  ...
  it('opens the dialog when the "Add Scene" button is clicked', async () => {
    ...
    const addSceneButton = screen.getAllByRole('button', { name: /مشهد جديد/i })[0];
